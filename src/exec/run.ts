...
const main = async () => {
    ...
    const networkMode = config.str('teleportation-network-mode', env.LIGHTBRIDGE_MODE)
    ...
    // TODO: OPTIONS
    if (!networkMode) {
        throw new Error('Must pass LIGHTBRIDGE_MODE, either testnets or mainnets explicitly')
    }
    const baseOpts: Omit<ILightBridgeOpts, "rpcUrl"> = {
        networkMode: networkMode?.toLowerCase() === ENetworkMode.TESTNETS ? ENetworkMode.TESTNETS : ENetworkMode.MAINNETS,
    ...
    const networksToWatch: IBobaChain[] = Object.values(BobaChains).filter((n: IBobaChain) => n.testnet === isTestnetMode)
    console.log("Watching networks:", networkMode, networksToWatch.map(n => n.name))

    const serviceWorkers: Promise<void>[] = []
    for (const network of networksToWatch) {
        const networkConfig = {...baseOpts, rpcUrl: network.url}
        serviceWorkers.push(piscina.run(networkConfig, {name: "startLightBridgeForNetwork"}))
